            ...
            case bencode:decode(BinaryData) of
                {ok, DecodedData} ->
                    ...
                    case maps:find(<<"info">>, DecodedData) of
                        {ok, InfoData} ->
                            InfoField =
                                #metainfo_info_field{name =
                                                         binary:bin_to_list(
                                                             maps:get(<<"name">>, InfoData, "")),
                                                     piece_len =
                                                         maps:get(<<"piece length">>, InfoData, 0),
                                                     pieces =
                                                         parse_pieces_data(maps:get(<<"pieces">>,
                                                                                    InfoData,
                                                                                    <<>>)),
                            ...
                            #metainfo{announce = Announce,
                                      ...
                                      info_hash = InfoHash};
                        error ->
                            erlang:error("Error: 'info' key not found in torrent file.~n", [])
                    end;
                error ->
                    erlang:error("Error: failed to decode bencoded data.~n", [])
            end;
        {error, Reason} ->
            erlang:error("Error: failed to read file: ~p~n", [Reason])
    end.
